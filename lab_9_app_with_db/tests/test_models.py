...
class TestCurrency(unittest.TestCase):
    ...
    def test_invalid_char_code_not_str(self):
        """Тест при ошибках в инициализации объектов класса Currency"""
        eur = Currency("EUR")
        with self.assertRaises(ValueError):
            Currency(123)
...
class TestSubscriptions(unittest.TestCase):
    ...
    def test_valid_subscription(self):
        """Тест при успешной инициализации объекта класса Subscriptions"""
        sub = Subscriptions(1, 1, [1, 2])
        self.assertEqual(sub.id, 1)
        self.assertEqual(sub.user_id, 1)
        self.assertEqual(sub.currency_id, [1, 2])

    def test_invalid_currency_id_not_list(self):
        """Тест при ошибке инициализации класса Subscriptions"""
        with self.assertRaises(ValueError):
            Subscriptions(1, 1, 1)
